(function(){
  ...
  const menu=document.querySelector('.menu-toggle'), links=document.querySelector('.nav__links'), mobileNavQuery=window.matchMedia('(max-width: 900px)');
  ...
  const closeNavSubmenus=()=>document.querySelectorAll('.nav__item--has-menu.is-open').forEach(item=>{item.classList.remove('is-open');const trigger=item.querySelector('.nav__trigger');if(trigger)trigger.setAttribute('aria-expanded','false')});
  if(menu&&links){const closeMenu=()=>{links.classList.remove('is-open');document.body.classList.remove('is-locked');menu.setAttribute('aria-expanded','false');menu.setAttribute('aria-label','Open menu');closeNavSubmenus()};menu.addEventListener('click',event=>{event.stopPropagation();const open=!links.classList.contains('is-open');links.classList.toggle('is-open',open);document.body.classList.toggle('is-locked',open);menu.setAttribute('aria-expanded',open?'true':'false');menu.setAttribute('aria-label',open?'Close menu':'Open menu');if(!open)closeNavSubmenus()});links.addEventListener('click',event=>{if(event.target===links)closeMenu()});document.addEventListener('keydown',event=>{if(event.key==='Escape'&&links.classList.contains('is-open'))closeMenu()});links.querySelectorAll('a').forEach(a=>a.addEventListener('click',event=>{if(mobileNavQuery.matches&&a.classList.contains('nav__trigger')){const item=a.closest('.nav__item--has-menu');if(item){event.preventDefault();const open=!item.classList.contains('is-open');closeNavSubmenus();item.classList.toggle('is-open',open);a.setAttribute('aria-expanded',open?'true':'false');return}}closeMenu()}))}
  document.querySelectorAll('.nav__item--has-menu').forEach(item=>{const trigger=item.querySelector('.nav__trigger');if(!trigger)return;let closeTimer=0;const setOpen=open=>{window.clearTimeout(closeTimer);trigger.setAttribute('aria-expanded',open?'true':'false');item.classList.toggle('is-open',open)};const scheduleClose=()=>{window.clearTimeout(closeTimer);closeTimer=window.setTimeout(()=>setOpen(false),360)};item.addEventListener('mouseenter',()=>{if(!mobileNavQuery.matches)setOpen(true)});item.addEventListener('mouseleave',()=>{if(!mobileNavQuery.matches)scheduleClose()});item.addEventListener('focusin',()=>{if(!mobileNavQuery.matches)setOpen(true)});item.addEventListener('focusout',event=>{if(!mobileNavQuery.matches&&!item.contains(event.relatedTarget))scheduleClose()})});
  ...
  const parallaxImages=['assets/images/parallax-tiles.webp','assets/images/process-roof-parallax.webp','assets/images/service-parallax-main.webp','assets/images/service-insight-parallax.webp'];
