(function(){
  ...
  const parallaxPanels=document.querySelectorAll('.parallax,.process-photo'); if(parallaxPanels.length&&window.matchMedia('(min-width: 901px)').matches){window.addEventListener('scroll',()=>{parallaxPanels.forEach(panel=>{const rect=panel.getBoundingClientRect();const shift=(window.innerHeight/2-(rect.top+rect.height/2))*.08;panel.style.backgroundPosition=`center calc(50% + ${shift}px)`})},{passive:true})}
